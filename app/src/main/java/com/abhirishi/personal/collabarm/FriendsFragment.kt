...
class FriendsFragment : Fragment(), FriendsListener {
    ...
    override fun onCreateView(inflater: LayoutInflater?, container: ViewGroup?,
        ...
        if (view is RecyclerView) {
            ...
            friendsRecyclerViewAdapter = FriendsRecyclerViewAdapter(getAllFriendsOnCollabarm(context), mListener)
            recyclerView.adapter = friendsRecyclerViewAdapter
        }
        return view
    }

    private fun getAllFriendsOnCollabarm(context: Context): List<Friend> {
        val friendsList = getAllFriends(context)
        checkForFriends(friendsList, this)
